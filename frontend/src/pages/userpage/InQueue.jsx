...
const InQueue = () => {
  ...
  const { setWindow, getTicketWindows, windowTicket } = useWindowStore();
        ...
        <div className="overflow-y-auto max-h-[400px] p-4 space-y-3">
          {queueData.length > 0 ? (
            queueData.map((window, index) => {
              const isUserTicket = window.window.id === ticket?.id; // Check if it's the user's ticket
              ...
              return (
                <div
                  ...
                  className={`flex flex-col sm:flex-row items-center sm:justify-between p-4 rounded-lg border shadow-sm transition hover:shadow-md ${
                    isUserTicket
                      ? "bg-green-100 border-green-300" // Highlight user's ticket
